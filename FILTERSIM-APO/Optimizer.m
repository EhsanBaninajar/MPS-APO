...
Global_var
%% objective Function
nsim=5;    % Number of function evaluation
...
%% Loading Ti
%Training Image
[Ti,Xsize_Ti,Ysize_Ti,Zsize_Ti,nbvar_Ti,namevar_Ti]=LoadGrid('ti_stonewall.sgems');
TiType=1; % "0"for categorical Ti and "1" for continious Ti
gapdatabase
%% SPSA
%parameter initialization
theta=[0.1; 0.1; 0.01];% Search template and Inner Patch dimensions, number of multiple grids, minimum number of replicates
thetamin=[0.1; 0.1; 0.01];
thetamax=[3; 0.3; 1];
p=3;  % Dimension of the parameters search space
...
alpha=0.602;
gamma=0.101;
stepsizemax=0.05; %to be sure the second step don't degrad the quality a lot
stage=0;
counter=0;
seccounter=1;
...
FunValue_MSE=zeros(nsim,NSPSA);
stepsizestor=zeros(NSPSA,3);
%%
for k=0:NSPSA-1
    ak=a/(k+1+A)^alpha;
    ck=c/(k+1)^gamma;
    ...
    if counter==5 && stage==0
        ...
        hold on
        subplot(4,1,1)
        line([k,k],[0,thetamax(1)*10],'color','red')
        subplot(4,1,2)
        line([k,k],[0,thetamax(1)*10],'color','red')
        subplot(4,1,3)
        line([k,k],[0,thetamax(2)*100],'color','red')
        subplot(4,1,4)
