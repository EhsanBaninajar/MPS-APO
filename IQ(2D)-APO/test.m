...
gapconddis=1; % amount of conditional data around the gaps
TiType=1;
gap_num=15;    % number of gaps
%% Loading Ti
%Training Image
[Ti,Xsize_Ti,Ysize_Ti,Zsize_Ti,nbvar_Ti,namevar_Ti]=LoadGrid('ti_lena_crop1.sgems');
gapdatabase
%% params values
tilesize = 10; %x(1) % Optimal size for the patch size is between 1/7 and 1/8 of the TI 
% (To get better realizations tilesize should be minimum)
tilesize_vary = 0; % Randomize patch size (max +-10% of patch size)
overlap = 4; %x(2) % Optimal size for the overlap region is between 1/3 and 1/4 of the tilesize
nbreplicates = 10; %x(3) % Standard Number of replicas = 5 to 10 to avoid "Verbatim Copy"
cond = 1; % Define whether Conditional or Unconditional simulation (0 or 1)
c = 500; % No of conditioning points (Don’t need to be defined if you load your Data file in the code)
w = 0.99; % x(4) % Conditioning weight (Range [0 1])
...
if cond > 0
    ...
    %Datagap=datasample(Datagap,500,'Replace',false);
...
figure;
scatter(Data(:,1),Data(:,2),20,Data(:,3))