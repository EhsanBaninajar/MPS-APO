...
%% Loading Ti
%Training Image
[Ti,Xsize_Ti,Ysize_Ti,Zsize_Ti,nbvar_Ti,namevar_Ti]=LoadGrid('lena_crop.sgems');
TiType=[1]; % "0"for categorical Ti and "1" for continious Ti
gapdatabase
%% SPSA
%parameter initialization
theta=[0.2; 0.1; 1];%tilesize, overlap, nbreplicates, conditioning weight
thetamin=[0.1; 0.1; 0.1];
thetamax=[1; 0.7; 2];
...
alpha=0.602;
gamma=0.101;
stepsizemax=0.02; %to be sure the second step don't degrad the quality a lot
stage=0;
counter=0;
seccounter=1;
...
for k=0:NSPSA-1
    ...
    if counter==5 && stage==0
        ...
        subplot(2,2,2)
        line([k,k],[0,thetamax(2)*thetamax(1)],'color','red')
        subplot(2,2,3)
        line([k,k],[0,thetamax(3)*10],'color','red')
%         subplot(2,2,4)
